...
void SO3CmdToMavros::so3_cmd_callback(const kr_mav_msgs::SO3Command::ConstPtr &msg)
{
  ...
  ROS_INFO_STREAM("THROTTLE initial: " << throttle);
  ROS_INFO_STREAM("f_des(1): " << f_des(1));
  ROS_INFO_STREAM("f_des(2): " << f_des(2));
  ROS_INFO_STREAM("R_cur(0, 2): " << R_cur(0,2));
  ROS_INFO_STREAM("R_cur(1, 2): " << R_cur(0,2));
  ROS_INFO_STREAM("R_cur(2, 2): " << R_cur(0,2));

  // Scale force to individual rotor velocities (rad/s).
  throttle = std::sqrt(throttle / num_props_ / kf_);
  ROS_INFO_STREAM("THROTTLE after sqrt: " << throttle);

  // Scaling from rotor velocity (rad/s) to att_throttle for pixhawk
  throttle = lin_cof_a_ * throttle + lin_int_b_;
  ROS_INFO_STREAM("THROTTLE after scaling from rotor vel: " << throttle);
  ...
  ROS_INFO_STREAM("THROTTLE after clamping: " << throttle);
